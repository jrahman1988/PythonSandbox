...
for i in cc:
    url="http://api.coronatracker.com/v5/analytics/trend/country?countryCode={}&startDate={}&endDate={}".format(i, startDate, endDate)
    r = requests.get(url)
...
# for j in cc:
#     fig, ax = plt.subplots()
    cv = pd.read_csv("~/Desktop/Learning/DataScience/TestData/TrendData_BD.csv")
    cv = cv.drop(columns=['total_deaths', 'total_recovered', 'country_code', 'country'])
    ...
    # plt.suptitle(today)
    # ax.legend(['Total Cases {0} --> Change from last week: {1}=(+{2}%)'.format(tc, tc_deltasincelastweek,tc_percentChangesincelastweek)], fancybox=True, framealpha=1, shadow=True, borderpad=1)
...
'''
    ...
    # ax = cv.plot(kind='hist', x='last_updated', y=['total_confirmed'] , rot=15, figsize=(16,6), grid=True, title="Trend of {}".format(countryName[k]), ax=ax)
'''
